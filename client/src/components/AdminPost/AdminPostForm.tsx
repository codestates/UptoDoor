...
function AdminPostForm(): React.ReactElement {
  ...
  const removeMenuHandler = (e: any): void => {
    if (menuArr.length > 1) {
      const filtering = menuArr.filter(
        (el: any) => el !== menuArr[e.target.id]
      );
  ...
  const submitHandler = (e: React.FormEvent<HTMLFormElement>): void => {
    e.preventDefault();
    if (adminAddressDetail.length === 0)
    ...
    if (
      ...
      dispatch(adminStorePost(adminPostInfo)).then((res: any) => {
        ...
        <Wrapper>
          <FlexBox>
            ...
            <AdminForm>
              ...
              {/* 가게 사업자등록증 파일업로드 */}
              <AdminFileUpload updateStoreFile={updateStoreFile} />
              ...
              <StoreBtnBox>
                ...
                <SmallButton type="button" onClick={handleClickCancel}>
                  {" "}
                  취소{" "}
                </SmallButton>
              </StoreBtnBox>
