...
function AdminOrderInfo({ orderItem, listbackHandler }: Iprops) {
  ...
  return (
    <AdminContainer>
        ...
        <StoreInfoWrapper className="storeinfo-wrapper">
          <FlexBox col>
            <EachItemBox>
              <H4>🗓 구독기간</H4>
              <P>
                {delivery_term}개월({Number(delivery_term) * 4}주) / 매주{" "}
                {delivery_day}요일 / {delivery_time}
              </P>
            </EachItemBox>
            <EachItemBox>
              <H4>📍 고객 주소</H4>
                  ...
                  <FlexBox align>
                    ...
                    <div className="order-text-content">
                      ...
                      <EachItemBox>
                        <H4>가격/수량</H4>
                        <P>
                          {stringToPrice(item.menu.price)} 원 / {item.quantity}
                          개
                        </P>
                      </EachItemBox>
                    </div>
      ...
      <BtnBox btnboxMargin>
        <MiddleButton primary onClick={listbackHandler}>
          확인
        </MiddleButton>
      </BtnBox>
    </AdminContainer>
