    ...
    <MypageOrderListWrapper>
      <OrderListContent>
            ...
            <EachItemBox>
              ...
              {orderitem.state === "cancel" ? (
                <P cancleline lightColorText>
                  {orderitem.delivery_term}개월(
                  {Number(orderitem.delivery_term) * 4}주) / 매주{" "}
                  {orderitem.delivery_day.map((day: any) => `${day}요일 `)} /
                ...
                <P>
                  {orderitem.delivery_term}개월(
                  {Number(orderitem.delivery_term) * 4}주) / 매주{" "}
                  {orderitem.delivery_day.map((ele: any) => `${ele}요일 `)} /
                  {orderitem.delivery_time} 시
                </P>
              )}
            </EachItemBox>
        ...
        <OrderInfoWrapper className="orderinfo-wrapper">
          ...
          {/* 오더인포 component */}
          <MyOrderDetailItem menus={orderitem.menu} />
      ...
      <BtnBox btnboxMargin>
        <SmallButton primary onClick={listbackHandler}>
          뒤로가기
        </SmallButton>
        {orderitem.state === "order" ? (
          <SmallButton onClick={cancelStoreHandler}>구독취소</SmallButton>
        ) : null}
      </BtnBox>
