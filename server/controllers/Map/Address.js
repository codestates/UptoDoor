...
module.exports = async (req, res) => {
    ...
    await user.update(
      {
        ...
        main_Xvalue: main_xvalue,
        main_Yvalue: main_yvalue,
        subAddress: subAddress,
        subAddressDetail: subAddressDetail,
        sub_Xvalue: sub_xvalue,
        sub_Yvalue: sub_yvalue,
      },
      { where: { id: id } }
    );
  } catch (err) {
  ...
  res
    .status(201)
    .send({
      message: "address check success",
      data: { mainAddress, mainAddressDetail, subAddress, subAddressDetail },
    });
};
