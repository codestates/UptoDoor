...
module.exports = async (req, res) => {
  ...
  const avg = term.map((el) => {
    return Number(el.delivery_term);
  });
  const termavg = avg.reduce((a, b) => {
    return a + b;
  });
  sendinfo.term = (termavg / avg.length).toFixed(1); //평균 구독 개월 소숫점 첫째자리
  //! 많이 찾는 카테고리별 순위
  const orderInstore = await order.findAll({
    raw: true,
    attributes: ["store_id"],
  });
  ...
  //! 많이 찾는 사용자 < 연령대, 성별 >
  const gender = await user.findAll({
    raw: true,
  ...
  const mandata = await user.findAll({
    where: { gender: "남자" },
    attributes: ["age"],
    include: [
      {
        ...
        include: [
          {
            model: order,
            attributes: ["store_id"],
            include: [{ model: store, attributes: ["category"] }],
          },
        ],
      },
    ],
  ...
  const womandata = await user.findAll({
    where: { gender: "여자" },
    attributes: ["age"],
    include: [
      {
  ...
  const addr = await order.findAll({
    raw: true,
    attributes: ["selected_address"],
  });
  let arr5 = []; //주소
  let arr6 = []; //주소 갯수
  let arr7 = [];
  ...
  addr.map((el) => {
    el.selected_address = el.selected_address.split(" ")[1];
  });
