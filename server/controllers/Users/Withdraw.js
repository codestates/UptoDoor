...
module.exports = async (req, res) => {
  ...
  let orderdata = await user.findOne({ where : { id : id } , attributes: ['id'],
  include : [{ model: user_order , attributes: ['order_id'], 
  ...
  orderdata.user_orders.map( async (el) => {
    if(el.order.order_deliveries.deliveries_term > el.order.order_deliveries.paycount){
      await axios.delete(`http://localhost:3060/cancel/${el.order_id}`)
      .then(() =>{
